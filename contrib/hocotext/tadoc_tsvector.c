...
*/
static char* get_word(char* source, uint32_t* word_len, char* parse_end) {
...
static void print_word(ParsedWord pw) {
	// printf("%u: ", pw.pos.pos);
 ...
 */
static int32 compare_string(char *a, int lena, char *b, int lenb, bool prefix)
{
	int			cmp;
...
static void lowercase_first(char* word) {
	if (*word < 'a') {
		*word += 'a' - 'A';
	}
}
...
static char* tsvector2text(HocotextTsvector* tsvector, uint32_t* ret_len) {
	...
	text* tsvector_text = (text*)palloc(alloc_len + VARHDRSZ);
	char* buffer = VARDATA_ANY(tsvector_text);
	...
	for (int i = 0; i < tsvector->num_diffwords; ++i) {
		...
		write_bytes += sprintf(buffer + write_bytes, "%c", SPLIT_CHAR);
		if (write_bytes > alloc_len / 2) {
			alloc_len *= 2;
			tsvector_text = (text*)repalloc(tsvector_text, alloc_len + VARHDRSZ);
			buffer = VARDATA_ANY(tsvector_text);
		}
	}
	*ret_len = write_bytes;
	SET_VARSIZE(tsvector_text, write_bytes + VARHDRSZ);
...
*/
text * tadoc_to_tsvector(char* tadoc_comp_data) {
	...
	parsed_text.lenwords = raw_size / 3;	
	if (parsed_text.lenwords < 100)
		parsed_text.lenwords = 100;
	...
	// printf("%s", "end make_tsvector\n");
	uint32_t ret_len;
	text* tsvector_text = tsvector2text(tsvector, &ret_len);
	// printf("tsvector2text: return length: %d\n", ret_len);
	return tsvector_text;
}