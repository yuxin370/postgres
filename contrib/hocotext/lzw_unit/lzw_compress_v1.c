...
#define buf_get_dict_entry(__bp,__front,__back)                                        \
do{                                                                     \
    ...
    hash_insert_rev(tmp,id,__front,__back);                                                \
}while(0)
...
void hash_insert_rev(char* ikey, int id, int front, int back) {
...
char * find_first_word(struct hash_entry_rev* prev){
    char * fw = malloc(sizeof *fw);
    struct hash_entry_rev* tp = prev;
    while(tp->front!=-1){
        tp = hash_find_rev(tp->front);
    } 
    return tp->key;
}
...
lzw_decompress(char* sp, int slen, char* dest, int rawsize){
    ...
    while(sp < srcend){
        ...
        tmp = hash_find_rev(cur_id);
        if(tmp){
            printf("find it (%s,%d) in dictionary\n",tmp->key,tmp->id);
            strcpy(cw,tmp->key);
            word_size = strlen(cw);
            strncpy(dp,cw,word_size);
            dp += word_size;
            char * from = find_first_word(tmp);
            printf("concating front = %s\n",from);
            strcat(pw,from);             
            hash_insert_rev(pw,entry_count++,front,back);
            front = back;
            prev = tmp;
        }else{
            char * from = find_first_word(prev);
            printf("concating front = %s\n",from);
            strcat(pw,from); 
            hash_insert_rev(pw,entry_count++,front,back);
            strcpy(cw,pw); 
...
int main(int argc, char *argv[])
{
    ...
    printf(" === decompressing completed ==== \n compressed len = %d, raw text len = %ld\n decompressed data : %s",reslen,rawlen,decomp_dest);
    hash_clear();
    return 0;
}