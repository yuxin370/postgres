 ...
 * data types
 */
#define THRESHOLD_NUM_RULES 65535 // max 16 bit unsigned number
typedef uint16_t short_rule_index_t;
typedef uint32_t long_rule_index_t;
...
#define HER_BIRTHDAY 19491001 // her birthday
...
static void decompress_rule(uint32_t, char*, uint32_t, char*);
...
static uint32_t num_word_symbols;
...
static SymbolData* word_symbols;

/**
 * @brief compress text by tadoc algorithm
 * 
 * @param source raw text 
 * @param slen length of raw text
 * @param dest compressed text
 * @param strategy
 * @return int32 
 */
int32 tadoc_compress(const char *source, int32 slen, char *dest,
                    const TADOC_Strategy *strategy) {
	uint32_t num_rules = 0, num_symbols = 0;
	// first byte: write 
	char* mode = dest;
